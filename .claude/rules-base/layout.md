...
Figma 返回的 `right` 坐标表示元素到右侧边缘的距离，**right 值越大 = 越靠左**：
...
**例子**：iPhoneX 右侧 icons
- Signal: right=64.33px → 视觉最左
- WiFi: right=44.03px → 中间
- Battery: right=14.67px → 视觉最右

Flexbox `flex-row` 布局时，DOM 顺序 = 视觉从左到右顺序（不需要 reverse）。
