...
为什么做：让 PM 能用自然语言快速搭出可上线的页面原型，最大化减少人工介入。
目前卡点追踪见 `docs/.claude/project-status.md`，当上下文丢失时读取。
...
定稿设计精调（Figma 插件导回，把 HTML demo 识别为 Figma 组件）            │
    ↓                                                              │
AI 翻译（figma-to-code 把设计稿翻译成生产代码，骨架曾书伟 / 翻译 Jocleyn） │
    ↓                                                              │
Dangoui 组件库（生成可用代码）─────────────────────────────────────────┘
    ↓
生产代码上线
...
| `.claude/rules-base/` | 翻译规则 | Jocelyn | `core.md` / `layout.md` / `_catalog.md` 等，定义组件映射 / 翻译工作流 |
| `figma-to-code/` | CLI + 骨架提取 | 曾书伟 | `figma-to-code` CLI；Props 统计见 `docs/.claude/props-reference.md` |
| `packages/` | 原子/分子组件 + 业务组件包 | 前端组 | `packages/dangoui`（原子分子）；`packages/islands` / `spu` / `purchase`（业务组件包） |
...
<img width="1080" src="https://assets.qiandaocdn.com/admin-upload/4d51d4a543def903c0dafe09e97afa81.png">
