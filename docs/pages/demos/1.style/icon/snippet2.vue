...
// 上传顺序 = 显示顺序，fallback 到 icons 键的插入顺序（JSON 里已按上传顺序排列）
const allNames = Object.keys(iconConfig.icons)
const linearIcons = computed(() => {
  return allNames.filter(n => n.includes(keyword.value) && !n.includes('filled'))
})
