...
| PriceEcho | `PriceEcho` | docs/business/PriceEcho.vue | 已生成 |
| PriceBasic | `PriceBasic` | docs/business/PriceBasic.vue | 已生成 |
| RateSmallSize | `RateSmallSize` | docs/business/RateSmallSize.vue | 已生成 |
| SPUFirstInfoSecond | `SPUFirstInfoSecond` | docs/business/SPUFirstInfoSecond.vue | 已生成 |
...
<DuNavigationBar>
  <template #left>
    <DuIconButton name="close" @click="handleClose" />
  </template>
  标题
</DuNavigationBar>
```
...
<DuTabs v-model:value="activeTab">
  <template #left><DuIcon name="filter" :size="16" /></template>
...
<DuBadge :value="5" color="error">
  <DuIcon name="notification" :size="24" />
</DuBadge>
...
```html
<DuImage src="imageUrl" width="100" height="100" radius="8" mode="aspectFill" />
<DuImage src="coverUrl" width="100%" height="200" mode="widthFix" />
```
...
<DuSearch v-model:value="keyword" placeholder="搜索">
  <template #left><DuIcon name="scan" :size="20" /></template>
...
<DuSwiper autoplay>
  <DuSwiperItem v-for="item in banners" :key="item.id">
    <DuImage :src="item.image" width="100%" height="150" />
  </DuSwiperItem>
</DuSwiper>
```
...
<!-- 翻译 -->
<DuImage :src="bannerUrl" width="100%" height="200" mode="aspectFill" />
```
...
<!-- 翻译 -->
<DuImage :src="item.cover" :width="80" :height="80" radius="8" mode="aspectFill" />
```
